...
static __always_inline struct sdprobe_bpf_settings *get_bpf_settings(void)
{
...
static __always_inline struct sdprobe_bpf_per_cpu_state *get_local_state(unsigned int cpu)
{
	struct sdprobe_bpf_per_cpu_state *state;
...
static __always_inline bool acquire_local_state(struct sdprobe_bpf_per_cpu_state *state)
{
	if (state->in_use) {
...
static __always_inline bool release_local_state(struct sdprobe_bpf_per_cpu_state *state)
{
	if (!state->in_use) {
...
static __always_inline void reset_tail_ctx(struct sdprobe_bpf_per_cpu_state *state,
					   enum ppm_event_type evt_type,
